...
async function oturum(req) {
  ...
  try {
    ...
    const r = await pool().query('SELECT id, ad, eposta, rol FROM uyeler WHERE id = $1', [o.uid]);
  ...
  try {
    switch (islem) {
      ...
      case 'dogrula': {
        ...
        const u = (await client.query('SELECT id, ad, eposta, rol FROM uyeler WHERE eposta = $1', [eposta])).rows[0];
        if (!u) return hata(res, 'Kayıt bulunamadı.', 404);
        const d = (await client.query('SELECT * FROM dogrulama WHERE eposta = $1 ORDER BY id DESC LIMIT 1', [eposta])).rows[0];
      ...
      case 'kod-tekrar': {
        const eposta = String(g('eposta', '')).trim().toLowerCase();
        const u = (await client.query('SELECT id, ad, eposta_onay FROM uyeler WHERE eposta = $1', [eposta])).rows[0];
        if (!u) return hata(res, 'Kayıt bulunamadı.', 404);
        if (u.eposta_onay) return hata(res, 'Bu e-posta zaten doğrulanmış.');
        const son = (await client.query('SELECT olusturma FROM dogrulama WHERE eposta = $1 ORDER BY id DESC LIMIT 1', [eposta])).rows[0];
      ...
      case 'giris': {
        const eposta = String(g('eposta', '')).trim().toLowerCase();
        const sifre = String(g('sifre', ''));
        const u = (await client.query('SELECT id, ad, eposta, parola, rol, eposta_onay FROM uyeler WHERE eposta = $1', [eposta])).rows[0];
        if (!u) return hata(res, 'Bu e-posta ile kayıt bulunamadı. Önce kayıt olun.', 404);
        if (!(await bcrypt.compare(sifre, u.parola))) return hata(res, 'Şifre hatalı. Tekrar deneyin.', 401);
        if (!u.eposta_onay) return hata(res, 'E-POSTA-DOGRULAMA-GEREK:E-postanıza gönderilen 6 haneli kodu girerek hesabınızı doğrulayın.', 403);
      ...
      case 'uyeler': {
        ...
        const l = await client.query('SELECT id, ad, eposta, rol, eposta_onay, olusturma FROM uyeler ORDER BY olusturma DESC');
